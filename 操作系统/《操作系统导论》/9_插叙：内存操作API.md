### Copilot said: 你好！作为笔记大师，我非常乐意帮你完善这份关于 **内存 API (Memory API)** 的笔记。这是 C

你好！作为笔记大师，我非常乐意帮你完善这份关于 **内存 API (Memory API)** 的笔记。这是 C 语言编程中最容易“翻车”的地方，也是面试和实战中的重灾区。

你的原始笔记结构清晰，覆盖了核心 API 和常见错误。我将通过**代码示例**来具象化这些错误，补充 `malloc` 底层的**元数据（Metadata）**概念，并解释为什么 `brk` 是底层支持。
